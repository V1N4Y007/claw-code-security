...
fn credentials_home_dir() -> io::Result<PathBuf> {
    if let Some(path) = std::env::var_os("VINAYCODE_CONFIG_HOME") {
        return Ok(PathBuf::from(path));
    }
    let home = std::env::var_os("HOME")
        .ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "HOME is not set"))?;
    Ok(PathBuf::from(home).join(".vinaycode"))
}
...
mod tests {
    ...
    fn oauth_credentials_round_trip_and_clear_preserves_other_fields() {
        ...
        std::env::set_var("VINAYCODE_CONFIG_HOME", &config_home);
        ...
        std::env::remove_var("VINAYCODE_CONFIG_HOME");
